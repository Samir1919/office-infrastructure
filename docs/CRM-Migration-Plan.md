...
MongoDB 8.3.4 is installed and validated on `db01`. The Vault-managed `crm_app` user has `readWrite` access to `crm_prod` only, and authenticated TCP `27017` access is restricted by UFW to `crm01` only. The Node.js 24 internal canary is validated on `crm01`. A copy from Windows `realestate_crm` to `crm_prod` imported 275 leads and 4 users; permission taxonomy mapping was applied and owner browser validation found no visible application problem. The owner confirmed that the Windows CRM received no new data after this migration, so `crm_prod` is the latest source copy known to the owner and no delta migration, write freeze, or `crm_restore_test` rehearsal is currently needed. The [CRM Production Cutover Runbook](CRM-Cutover-Runbook.md) remains a contingency if Windows writes resume. The Windows source remains unchanged, current-database backup protection is the next preparation task, and public publication remains separately gated.
