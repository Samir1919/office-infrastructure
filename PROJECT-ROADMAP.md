...
- The internal CRM canary is deployed on `crm01` from Git revision `e7a9ddbf8e8e3b12ba187906484e813150a3490f`, runs Node.js 24 LTS, returns healthy from `/healthz`, connects to `crm_prod`, and has no Nginx Proxy Manager host, public DNS, TLS certificate, or router forwarding. Permission taxonomy mapping remains applied to the migrated users.
- The approved encrypted MongoDB-backed 12-hour session store is active. Machine validation confirmed the `crm_prod.sessions` collection, its TTL index, and unchanged counts of 275 leads and 4 users. The remaining gate is an owner browser login followed by an application-container restart and same-browser refresh. Future CRM database archives exclude the ephemeral `sessions` collection so recovery intentionally requires a fresh login.
...
5. Complete the remaining session-store browser gate: owner login on the internal canary, application-container restart, same-browser authenticated refresh, and final resource/log validation. Public implementation remains blocked on rate limiting, security headers, staged NPM/TLS validation, and separate owner approvals. Hardware work remains deferred.
