...
5. Fix and validate the owner-reported login-form keyboard issue where pressing Enter after credentials does not reliably submit, while clicking Login works. After that focused fix, continue the public-readiness sequence with rate limiting, security headers, staged NPM/TLS validation, and separate owner approvals. Hardware work remains deferred.
