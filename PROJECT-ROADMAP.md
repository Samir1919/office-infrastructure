...
5. CRM database and application recovery documentation is complete. The publication plan is prepared; the owner must now choose internal-only, VPN-only, or eventual public HTTPS. Public implementation remains blocked until that choice and the documented session-store/security decisions are approved. Hardware work remains deferred.
