...
5. CRM database and application recovery documentation is complete at the current non-destructive validation level. Continue read-only monitoring. The next owner decision is whether to keep the CRM internal-only or begin documentation-first Nginx Proxy Manager/HTTPS publication planning; no publication change is approved yet, and hardware work remains deferred.
