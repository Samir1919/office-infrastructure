...
- CRM revision `e7a9ddbf8e8e3b12ba187906484e813150a3490f` containing the approved encrypted MongoDB-backed 12-hour session store has passed application tests and CI and is pinned for the next internal canary deployment. The deployed revision remains `ae9539ca575df9ffdafe047c49b20fff2473b858` until the canary apply and validation complete. Future CRM database archives exclude the ephemeral `sessions` collection so recovery intentionally requires a fresh login.
...
5. Deploy and validate the approved MongoDB-backed session revision on the internal `crm01` canary without resetting `crm_prod`; verify browser login persistence across an application-container restart, TTL cleanup, unchanged CRM record counts, and resource health. Public implementation remains blocked on rate limiting, security headers, staged NPM/TLS validation, and separate owner approvals. Hardware work remains deferred.
