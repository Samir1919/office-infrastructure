...
- `db01` will use MongoDB `8.2.11`, the approved current stable patch release. The new CRM production database is `crm_prod`; the confirmed Windows source is MongoDB `7.0` database `realestate_crm`. Its document-upload storage method must be confirmed before migration.
