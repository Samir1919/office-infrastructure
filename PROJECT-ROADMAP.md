...
- The owner approved the next internal-canary hardening step: login-only rate limiting with 5 failed account-and-IP attempts and 25 failed IP-wide attempts per 15 minutes, plus Helmet security headers. HSTS remains disabled on internal HTTP, CSP enforcement is deferred until inline browser code is migrated, and public/NPM/DNS/router work remains separately gated.
- CRM pull request #5 passed test/build/security and visual-regression CI, merged as revision `55331b096fa64b7fde8d505cc9dd209935b6b5b7`, and is approved as the next `crm01` canary target. Rollback remains the validated revision `1a8301bca2b4b57bd40a4847b0f83aaa40c6b341`.
...
- CRM access/publication architecture review is complete. The current application is reverse-proxy-aware and supports secure cookies, but its default in-memory session store and absence of documented login rate limiting/security-header middleware block unrestricted public exposure. Internal-only remains the safe default; VPN-only is the recommended near-term remote option. Public HTTPS requires application hardening, NPM/DNS/TLS/edge facts, staged validation, and separate owner approvals.
...
5. Continue the public-readiness application-hardening sequence with documented rate limiting and security headers. Staged NPM/TLS validation and any public exposure require separate owner approvals. Hardware work remains deferred.
